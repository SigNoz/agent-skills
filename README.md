...
| [signoz-agent-multi-agent-triage](plugins/signoz/skills/signoz-agent-multi-agent-triage/SKILL.md) | Diagnose why a specific agent in a multi-agent fleet failed or stalled by correlating its gen_ai spans, upstream handoff chain, and token anomalies in SigNoz, then ranking likely causes and suggesting a fix. |
