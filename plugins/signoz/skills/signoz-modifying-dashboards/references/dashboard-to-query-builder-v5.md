...
- If an execution-affecting field has no equivalent in the current tool schema,
  ...
  the resulting query validated. Only continue to update the dashboard if the
...
not clear the safety gate. Before updating the dashboard, stop or obtain the
...
The nested `dashboard` object sent to `signoz_update_dashboard` remains
dashboard/editor JSON. Keep `queryName`, `dataSource`, `filters`, `pageSize`,
`orderBy`, `selectColumns`, dashboard HAVING clauses, `queryTraceOperator`, and
`groupBy` fields such as `key`/`dataType`/`type`. Execution-only names belong only in
